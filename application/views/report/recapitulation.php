	...
	<div class="container-fluid">
		<div class="row bg-light pt-3 pb-2">
			...
			<div class="col-4 text-right">
				...
				<a href="<?=base_url('report/export_result_subject/'.$uc_period)?>" class="btn btn-sm btn-primary mx-4"><i class="fa fa-file-export"></i> &nbsp; Export Result</a>
			</div>
			-->
							...
							<?php foreach ($part as $part) : ?>
								...
								<tr>
									...
									<?php foreach ($comp as $co) : ?>								
										<td class="text-center">
											...
											if (isset($score[$part['seafarer_code']][$co->uc])) {
												if (($score[$part['seafarer_code']][$co->uc] == "NA") ||
													...
													$the_att = $att[$part['seafarer_code']][$co->uc];
												} 
											}
										...
										?>
										<?php if ($the_att != NULL) : ?>
											<a href="<?=base_url('report/show_answer/'.$uc_period.'/'.$co->uc.'/'.$the_att)?>" target="_blank" class="<?=$text_color?>"><?=$the_score?></a>
										<?php else : ?>
											<span class="<?=$text_color?>"><?=$the_score?></span>	
										<?php endif; ?>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<?=form_open_multipart('comeon/magic')?>
					...
					<input type="hidden" name="f_redirect" value="recap" />
					<!-- Modal Header -->
