...
<div class="container-fluid">
	<div class="row bg-danger pt-3 pb-2">	
		<div class="col-3 text-left">
			<a href="<??>"><i class="fa fa-trash-alt text-warning"></i></a>
		</div>
	...
	<div class="row mt-3 font-weight-bolder text-primary">
		...
		<div class="col-2">
			<?php
				...
				$ex_status = ($ex_status == 1 ? "Perdana" : "Mengulang");
			?>
	...
	<div class="row mt-2">
		<?php if (isset($comp)) : ?>
			...
			<table class="table table-bordered table-striped" style="font-size: 0.9em">
				<thead>
					<tr class="thead-dark">
						...
						<?php foreach ($comp as $co) : ?>
							<th class="rotated-text" width="50"><div><?=word_limiter($co->label,6)?></div></div></th>
						<?php endforeach; ?>	
					</tr>
					<tr class="thead-dark text-center">
						...
						<?php foreach ($comp as $co) : ?>
							<?php
								...
								if (isset($exam[$xco])) {
									$xcom = $exam[$xco];
									$thex = substr($xcom, 0, 2)."".substr($xcom, 10, 2)."".substr($xcom, 13, 2);
								}
								else {
									$thex = "";
								}
							...
							?>
							<th style="background-color: #<?=$thex?>"><?=$no?></th>
							<?php $no++; ?>
						<?php endforeach; ?>
					</tr>

					<tr class="thead-dark text-center" style="background-color: #809800;">
						<?php foreach ($comp as $co) : ?>
							<th>
								<?php if (isset($exam[$co->uc])) : ?>

									<a href="<?=base_url('report/regattemer/'.$uc_period."/".$co->uc)?>" title="Regenerate">
				...
				<?=form_open('report/clear_participant')?>
					...
					<?php if (isset($part)): ?>
						<?php foreach ($part as $part) : ?>
							...
							<tr>
								...
								<td><?=$part['seafarer_code']?>
									<input type="checkbox" name="f_seaf_del[]" value="<?=$part['seafarer_code']?>" class="ml-3">
								</td>
								...
								<?php foreach ($comp as $co) : ?>								
									<td class="text-center">
										...
										<a href="<?=base_url('report/regatt_single/'.$uc_period.'/'.$part['seafarer_code'].'/'.$co->uc)?>" target="_blank" class="<?=$text_color?>"><?=$the_score?></a>
									</td>
								<?php endforeach; ?>
				...
				<input type="submit" name="f_clear" value="Clear Participant">
				<?=form_close()?>
			</table>
